...
use std::env;
use std::fs;
use vibegraph::{AppConfig, IndexingConfig, Vibegraph};
...
/*
    ...
    This is the VERY first bot and most important bot of all to run.  This collects events from the chain with vibegraph which are needed for all other bots !
...
async fn main() {
    env_logger::init();
    dotenv().ok();
    run_vibegraph_bot().await;
}
pub async fn run_vibegraph_bot() {
    ...
    let db_conn_url = std::env::var("DB_CONN_URL").expect("RPC_URL must be set");
    ...
    let event_indexer_table_name = Some("event_indexers".to_string());
